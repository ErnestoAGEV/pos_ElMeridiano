...
- [x] **Step 4:** Commit: `feat: tamaño de etiqueta configurable en config_tienda`
...
- [x] **Step 2:** Handler de Enter (coincidencia exacta de código, case-insensitive):
...
- [x] **Step 3:** Conectar al input de búsqueda: `ref={searchRef}` y `onKeyDown={handleBusquedaKeyDown}` (el input ya tiene `autoFocus`).
- [x] **Step 4:** Re-enfocar búsqueda al cerrar los modales de pieza/precio y al completar venta: `searchRef.current?.focus()` en `handleAgregarPieza`, `handleAgregarConPrecio` y en los `onClose` de ambos modales.
- [x] **Step 5:** Verificación manual: teclear código exacto + Enter agrega al carrito (precio fijo) o abre modal de peso (oro/plata); código inexistente muestra toast y selecciona el texto.
...
- [x] **Step 1:** Importar ícono `Barcode` de lucide-react y `EtiquetaModal`; agregar estado `etiquetaModal`.
- [x] **Step 2:** Botón junto a editar/eliminar en cada tarjeta (con `e.stopPropagation()`), abre el modal con el producto.
...
- [x] **Step 2:** Nueva sección "Etiquetas de producto" con presets (botones: Dumbbell 50×10, Rectangular 30×20, Rectangular 40×30) que fijan ambos campos, + inputs numéricos de ancho/alto en mm para personalizado.
- [x] **Step 3:** Validación en `handleSave`: ancho ≥ 10, alto ≥ 5; si no, toast de error y no guardar.
- [x] **Step 4:** Commit: `feat: tamaño de etiqueta configurable en personalizacion`

### Task 7: Verificación final

- [x] **Step 1:** `npm run build` sin errores.
- [x] **Step 2:** Prueba manual del flujo completo en `npm run electron:dev`: configurar tamaño → imprimir etiqueta (Guardar como PDF, verificar tamaño de página) → escanear/teclear código en POS.
- [x] **Step 3:** Commit final si quedó algo pendiente.
